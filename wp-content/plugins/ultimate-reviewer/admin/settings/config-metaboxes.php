<?php if ( ! function_exists( 'gpur_metaboxes_settings' ) ) {
	function gpur_metaboxes_settings() {
		...
		if ( 'allowed' === gpur_permissions( 'specific-roles', gpur_option( 'review_management' ) ) ) { 
			...
			if ( ! isset( $_GET['post_type'] ) OR ( isset( $_GET['post_type'] ) && 'gpur-template' !== $_GET['post_type'] ) ) {
				...
				$template_data = gpur_get_review_template_fields( $post_id );
				...
				if ( 0 == $template_data['site_max_rating'] ) {
					...
					if ( isset( $criteria ) && is_array( $criteria ) ) {
						...
						for( $i = 0; $i < $criteria['count']; $i++ ) {
							...
							$review[] = array(
								...
								'max' => $template_data['site_max_rating'],
								'default' => 0,
							);	
						}
					...
					if ( isset( $criteria ) && is_array( $criteria ) ) {
						...
						$classes = 'gp-setting-gp-gpur-manually-add-overall-rating gp-not-empty gp-hide';
					} else {
				...
				$settings[] = array(
					...
					'post_types' => gpur_option( 'review_post_types' ),
					'position' => 'normal',
					'priority' => 'high',
					'section' => $review
				);
			...
			$template = array(
				...
				array(
					...
					'type' => 'text',			
					'class' => 'gp-setting-gp-gpur-review-template-automatic-insertion gp-not-empty gp-hide',
				),
