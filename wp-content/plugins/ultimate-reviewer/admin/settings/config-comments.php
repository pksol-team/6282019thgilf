<?php if ( ! class_exists( 'GPUR_Comments_Backend' ) ) {
	class GPUR_Comments_Backend {
		...
		public function enqueue_scripts() {
			...
			wp_enqueue_style( 'gpur-comments', plugin_dir_url( __FILE__ ) . 'assets/comments.css', array(), GPUR_VERSION );	
		...
		public function register_settings( $comment ) {
			...
			<table id="gpur-review-details" class="form-table editcomment">
				<tbody>
					...
					<tr>		
						<td class="first">
							<label for="rating"><?php echo gpur_option( 'comment_form_rating_field_label' ); ?></label>
						</td>
						<td>	
		...
		public function verify_settings( $commentdata ) {
			...
			if ( 'disallowed' === gpur_permissions( gpur_option( 'comment_form_permissions' ), gpur_option( 'comment_form_permission_roles' ) ) ) {
				...
				if ( 'enabled' === gpur_option( 'comment_form_review_title' ) && isset( $_POST['gpur_title'] ) && '' === $_POST['gpur_title'] && '' === $commentdata['comment_parent'] ) {
					
					wp_die( esc_html__( 'Error: You must give a review title.', 'gpur' ), 'Error', array( 'response' => 500, 'back_link' => true ) );
					
				} elseif ( isset( $_POST['gpur_rating'] ) && 0 === $_POST['gpur_rating'] && gpur_option( 'comment_form_min_rating' ) > 0 && '' === $commentdata['comment_parent'] ) {
					
					wp_die( esc_html__( 'Error: You must give a rating.', 'gpur' ), 'Error', array( 'response' => 500, 'back_link' => true ) );
					
				} elseif ( 'one-rating-one-comment' === gpur_option( 'comment_form_comment_rating_limit' ) && ( ( isset( $_COOKIE['gpur_user_rating_' . $commentdata['comment_post_ID']] ) && ! is_user_logged_in() ) OR get_user_meta( get_current_user_id(), $ind_user_rating_meta_key, true ) ) ) {
		...
		public function edit_settings( $comment_id ) {
			...
			if ( ( isset( $_POST['gpur_up_votes'] ) ) && ( $_POST['gpur_up_votes'] != '' ) ) {
				$up_votes = floatval( $_POST['gpur_up_votes'] );
		 ...
		 *
		 */	
		public function delete_settings( $comment_id, $comment ) {
			...
			$rating_value = get_comment_meta( $comment_id, $avg_user_rating_meta_key, true );
		...
		public function rating_column( $column, $comment_ID ) {
			if ( $column === 'rating' ) {
				...
				echo '<p class="comment-rating">' . GPUR_Show_Rating::gpur_show_rating( 
				$comment_ID,
				'comment',
				array(
					'data' => 'comment-rating',
		...
		public function unapproval_to_approval( $comment ) {
			...
			if ( is_email( $comment->comment_author_email ) && get_comment_meta( $comment->comment_ID, 'gpur_rating', true ) ) {
