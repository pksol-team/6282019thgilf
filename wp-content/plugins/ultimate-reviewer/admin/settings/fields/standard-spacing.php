	...
	if ( is_array( $default ) ) {
		...
		foreach( $default as $k => $v ) {
			...
			echo '<span class="gp-styling-field gp-dimensions-field ' . $field_class . '">';
				if ( $count > 1 ) {
					echo '<label for="' . esc_attr( $new_id ) . '" class="gp-label">' . $title . '</label>';
				}	
				echo '<input type="text" id="' . esc_attr( $new_id ) . '" name="' . $new_name . '" class="gp-input-text gp-small-text" value="' . esc_attr( $new_value ) . '" /><span class="gp-units">' . esc_attr( $units ) . '</span>';
			echo '</span>';
