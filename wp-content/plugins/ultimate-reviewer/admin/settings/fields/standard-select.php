...
echo '<span class="gp-select-field">';
	...
	if ( 'multi' !== $format ) {
		...
		echo '<select id="' . esc_attr( $id ) . '" class="post_form' . $select2_class . '" name="' . esc_attr( $name ) . '"' . $link . '>';
		...
		echo '<select id="' . esc_attr( $id ) . '" class="post_form' . $select2_class . '" name="' . esc_attr( $name ) . '[]" multiple' . $link . '>';
			foreach( $options as $option_value => $option_title ) {	
