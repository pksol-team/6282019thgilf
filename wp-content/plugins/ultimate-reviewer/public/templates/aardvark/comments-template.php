...
<?php if ( comments_open() OR have_comments() ) { ?>
	...
	<div id="comments" class="comments-area">
		...
		$comment_args = array(
			...
			'must_log_in' => '<p class="must-log-in">' . sprintf( wp_kses( __( 'You must be <a href="%s">logged in</a> to post a comment.', 'gpur' ), array( 'a' => array( 'href' => array() ) ) ), ghostpool_login_link() ) . '</p>',
			...
			'fields' => apply_filters( 'comment_form_default_fields', array(
				...
				'url' => '<p class="comment-form-url new-hide"><input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" placeholder="' . esc_html__( 'Website', 'gpur' ) . '" /></p>'
