<?php if ( ! function_exists( 'gpur_good_points' ) ) {

	function gpur_good_points( $atts, $content = null ) {	
		...
		<div id="<?php echo esc_attr( $unique_id ); ?>" class="<?php echo esc_attr( $css_classes ); ?>">
			...
			if ( ! empty( $good_points ) ) { ?>
				...
				<ul class="gpur-good-list">
				
					<?php foreach( $good_points as $good_point ) { ?>
						<li><i class="<?php echo esc_attr( $icon ); ?>"></i><?php echo esc_attr( $good_point ); ?></li>
					<?php } ?>
...
add_shortcode( 'gpur_good_points', 'gpur_good_points' );