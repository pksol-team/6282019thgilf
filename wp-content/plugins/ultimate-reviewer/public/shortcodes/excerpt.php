<?php if ( ! function_exists( 'gpur_excerpt' ) ) {

	function gpur_excerpt( $atts, $content = null ) {	
		...
		// Text
		if ( $text_size ) {
			$inline_css .= '#' . esc_attr( $unique_id ) . ' .gpur-excerpt-text {font-size: ' . ghostpool_add_units( $text_size ) . ';}';
		} 
		if ( $text_color ) {
			$inline_css .= '#' . esc_attr( $unique_id ) . ' .gpur-excerpt-text {color: ' . esc_attr( $text_color ) . ';}';
		} 
		if ( $text_extra_css ) {
			$inline_css .= '#' . esc_attr( $unique_id ) . ' .gpur-excerpt-text {' . esc_attr( $text_extra_css ) . '}';
		}
		...
		<div id="<?php echo esc_attr( $unique_id ); ?>" class="<?php echo esc_attr( $css_classes ); ?>">
			...
			<div class="gpur-excerpt-text">
				<?php echo wp_kses_post( get_post_meta( get_the_ID(), $excerpt_meta_key, true ) ); ?>
			</div>
...
add_shortcode( 'gpur_excerpt', 'gpur_excerpt' );